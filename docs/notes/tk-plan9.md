# Tk on Plan 9, and Tk's own test suite

Split out of `CLAUDE.md` (2026-09). The `sys/src/external/tk/plan9/` backend
and every round of Tk's suite from run 1 to run 16 (178 failures, all 97 files,
clean exit). Also carries two rules found here that apply tree-wide: the
`HFILES` trap, and syntax-checking the backend on the host with gcc.
