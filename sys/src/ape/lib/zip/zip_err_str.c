/*
  Generated from lib/zip.h and lib/zipint.h; make changes there.

  Upstream builds this with CMake -- cmake/GenerateZipErrorStrings.cmake
  -- which is not available here, so it is generated once and kept in
  the tree beside the mkfile, the same way curl_config.h is. The
  generator is a transcription of that CMake script's two regexes; if
  the error tables in zip.h or zipint.h ever change, this file has to be
  regenerated to match. lib/zip_error_strerror.c indexes
  _zip_err_str[] by error code, so a stale table here is an out-of-range
  read, not a wrong message.
*/
