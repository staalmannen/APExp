...
/* Compute y+TAIL = log(x) where the rounded result is y and TAIL has about
   additional 15 bits accuracy.  IX is the bit representation of x, but
...
static inline double_t log_inline(uint64_t ix, double_t *tail)
{
	...
	ar2 = r * ar;
	ar3 = r * ar2;
	/* k*Ln2 + log(c) + r + A[0]*r*r + ar3*(A[1] + r*A[2]
...
double pow(double x, double y)
{
	...
	uint64_t ix, iy;
	uint32_t topx, topy;
	double_t lo, hi, ehi, elo;

	ix = asuint64(x);
	iy = asuint64(y);
	topx = top12(x);
	topy = top12(y);
	if (predict_false(topx - 1 >= 0x7ff - 1 ||
		...
		if ((topy & 0x7ff) - 0x3be >= 0x43e - 0x3be) {
			/* Note: sign_bias == 0 here because not all cases are
			   handled above.  */
			if (ix == asuint64(1.0))
				return 1.0;
			if ((topy & 0x7ff) < 0x3be) {
		...
		if (topx == 0) {
			...
			ix = asuint64(fp_barrier(x) * 0x1p52);
			ix &= 0x7fffffffffffffff;
			ix -= 52ULL << 52;
		}
	}
	...
	double_t llo = hi - lhi;
	ehi = yhi * lhi;
	elo = y * lo + (yhi * llo + ylo * hi); /* |elo| < |ehi| * 2^-25.  */
#endif
