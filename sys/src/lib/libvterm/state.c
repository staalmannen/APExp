...
/* ported: kencc cpp has no #if. DEBUG never defined here. */

/* Some convenient wrappers to make callback functions easier */

static void putglyph(VTermState *state, const uint32_t chars[], int width, VTermPos pos)
{
  /* ported: kencc rejects designated initializers of bitfield members */
  VTermGlyphInfo info;
  ...
  if(rect.start_col == 0 && rect.end_col == state->cols && rightward == 0) {
    ...
    if(downward > 0) {
      ...
      for(int row = rect.end_row - downward; row < rect.end_row; row++)
        memset(&state->lineinfo[row], 0, sizeof state->lineinfo[row]); /* ported: no compound literals */
    }
    else {
...
static void osc_selection(VTermState *state, VTermStringFragment frag)
{
  ...
  if(!frag.len) {
    ...
    if(frag.final && state->selection.callbacks->set) {
      VTermStringFragment sf; /* ported: no compound literals */
      memset(&sf, 0, sizeof sf);
      sf.str     = NULL;
      sf.len     = 0;
      sf.initial = state->tmp.selection.state != SELECTION_SET;
      sf.final   = true;
      (*state->selection.callbacks->set)(state->tmp.selection.mask, sf, state->selection.user);
    }
    return;
  }
    ...
    while((state->selection.buflen - bufcur) >= 3 && frag.len) {
      ...
      if(!frag.len || (state->selection.buflen - bufcur) < 3) {
        if(bufcur) {
          VTermStringFragment sf; /* ported: no compound literals */
          memset(&sf, 0, sizeof sf);
          sf.str     = state->selection.buffer;
          sf.len     = bufcur;
          sf.initial = state->tmp.selection.state == SELECTION_SET_INITIAL;
          sf.final   = frag.final && !frag.len;
          (*state->selection.callbacks->set)(state->tmp.selection.mask, sf, state->selection.user);
  ...
  if(state->callbacks && state->callbacks->resize) {
    ...
    if(rows != state->rows) {
      for(int bufidx = BUFIDX_PRIMARY; bufidx <= BUFIDX_ALTSCREEN; bufidx++) {
        ...
        for( ; row < rows; row++) {
          memset(&newlineinfo[row], 0, sizeof newlineinfo[row]); /* ported: no compound literals */
        }
