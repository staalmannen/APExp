 ...
 *
 * $Id: objc.m.in,v 1.9 2023/02/04 16:11:23 stes Exp $
 *
 * DO NOT EDIT -- configure objc-3.4.13 x86_64-unknown-plan9
 */
 ...
 * Defaults to "OBJCDIR" as set in configure, or environment variable.
...
dfltobjcdir(void)
{
  ...
  /* use OBJCDIRC, not OBJCDIR here (for double backslash) */
...
{
  /* undefine -U flag */
  return [String sprintf:"-U%s", element];
}
...
dfltargs(void)
{
  cpargs = addifnonempty([OrdCltn new], "-D__PORTABLE_OBJC__ -std=gnu17 ");
  ocargs = addifnonempty([OrdCltn new], "-gnu -noTags");
  ccargs = addifnonempty([OrdCltn new], "-std=gnu17 ");
...
pversion(void)
{
  printf("Portable Object Compiler - objc-3.4.13 - x86_64-unknown-plan9\n");
...
setglobals()
{
  ...
  if (attrcommon) {
    [ocargs add:[String str:"-attrcommon"]];
  }
...
drivercpp(id input, id extension, id result)
{
  ...
  if ([extension isEqualSTR:"c"]) {
    ...
    id tmpc = replacesuffix(input, extension, "c");
...
driverobjc(id name, id extension, id input, id result)
{
  ...
  success = drvcmd(objc1, NULL, cargs, nil);
...
/* this is for compiling _postlnk.c etc. */
...
processM(id input, id extension)
{
  ...
  tmpi = replacesuffix(input, extension, "i");
  if (tmpdir)
