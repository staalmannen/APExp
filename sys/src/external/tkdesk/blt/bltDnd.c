...
typedef struct AnyWindow {
    ...
    struct AnyWindow *parentPtr; /* Parent node. NULL if root. Used to
...
typedef struct {
    ...
    Tcl_HashTable handlerTable;	/* Table of data handlers (converters)
 ...
 */
static Blt_Chain *
GetWindowZOrder(display, parent)
    Display *display;
    HWND parent;
{
    Blt_Chain *chainPtr;
    ...
    LPARAM lParam;
{
    ...
    switch (message) {
    case WM_USER:
	result = GenerateDragEvent(hwnd, message, wParam, lParam);
	break;
    ...
    ClientData clientData;	/* widget data */
{
	...
	result = Tcl_Eval(srcPtr->interp, ExpandPercents(srcPtr->siteCmd, 
		subs, 2, &cmdStr));
    ...
    int *newPtr;		/* returns non-zero => new record created */
{
    ...
    hPtr = Tcl_CreateHashEntry(&sourceTable, (char *)tkwin, newPtr);
    if (!(*newPtr)) {
    ...
    srcPtr->hashPtr = hPtr;
    Tcl_InitHashTable(&(srcPtr->handlerTable), TCL_STRING_KEYS);
     ...
     *  is destroyed.
     */
    Tk_CreateEventHandler(tkwin, StructureNotifyMask, SourceEventProc, 
	(ClientData)srcPtr);
    return srcPtr;
}

    ...
    Source *srcPtr;
{
    ...
    for (hPtr = Tcl_FirstHashEntry(&(srcPtr->handlerTable), &cursor);
    ...
    Tcl_DeleteHashTable(&(srcPtr->handlerTable));
    ...
    Tk_Window tkwin;		/* Widget pathname for desired record */
{
    ...
    hPtr = Tcl_FindHashEntry(&targetTable, (char *)tkwin);
    if (hPtr == NULL) {
	return NULL;
    }
    ...
    Tk_Window tkwin;		/* Widget pathname for desired record */
{
    ...
    Tcl_InitHashTable(&(targetPtr->handlerTable), TCL_STRING_KEYS);
    targetPtr->hashPtr = Tcl_CreateHashEntry(&targetTable, (char *)tkwin, 
	     &isNew);
    Tcl_SetHashValue(targetPtr->hashPtr, (ClientData)targetPtr);
    ...
    DestroyData data;
{
    ...
    for (hPtr = Tcl_FirstHashEntry(&(targetPtr->handlerTable), &cursor);
	hPtr != NULL; hPtr = Tcl_NextHashEntry(&cursor)) {
	cmd = (char *)Tcl_GetHashValue(hPtr);
	if (cmd != NULL) {
	    free((char *)cmd);
    ...
    Tcl_DeleteHashTable(&(targetPtr->handlerTable));
    ...
    register Source *srcPtr;	/* drag&drop source record */
{
    ...
    /* Let TkDesk know we found a valid drop target */
    Tcl_SetVar2(srcPtr->interp, "tkdesk", "drop_on_item", "1",
		TCL_GLOBAL_ONLY);
	...
	hPtr = Tcl_FirstHashEntry(&(srcPtr->handlerTable), &cursor);
	dataType = Tcl_GetHashKey(&(srcPtr->handlerTable), hPtr);
    } else {
	hPtr = Tcl_FindHashEntry(&(srcPtr->handlerTable),
...
InitRoot(srcPtr)
    Source *srcPtr;
{
    srcPtr->rootPtr = (AnyWindow *) calloc(1, sizeof(AnyWindow));
    ...
    int x, y;
{
  ...
  descend:
	...
	rootPtr = (AnyWindow *) Blt_ChainGetValue(linkPtr);
	if (!rootPtr->initialized) {
	    QueryWindow(srcPtr->display, rootPtr);
	}
	if ((x >= rootPtr->x1) && (x <= rootPtr->x2) &&
    ...
    AnyWindow *windowPtr;		/* window rep to be initialized */
{
	...
	for (linkPtr = Blt_ChainFirstLink(chainPtr); linkPtr != NULL;
	    linkPtr = Blt_ChainNextLink(linkPtr)) {
	    childPtr = (AnyWindow *) calloc(1, sizeof(AnyWindow));
	    assert(childPtr);
    ...
    Target *targetPtr;		/* drag&drop target window data */
{
	...
	Tcl_DStringAppendElement(&dString,
	    Tcl_GetHashKey(&(targetPtr->handlerTable), hPtr));
    }
    ...
    char **argv;
{
	...
	nActive--;		/* One fewer active token window. */
    }
    return TCL_OK;
}
    ...
    char **argv;
{
    ...
    Tcl_SetResult(interp, (nActive > 0) ? "1" : "0", TCL_STATIC);
    return TCL_OK;
}
    ...
    char **argv;
{
	...
	for (hPtr = Tcl_FirstHashEntry(&(srcPtr->handlerTable), &cursor);
		...
		Tcl_GetHashKey(&(srcPtr->handlerTable), hPtr));
	}
	return TCL_OK;
    }
    hPtr = Tcl_CreateHashEntry(&(srcPtr->handlerTable), argv[4], &isNew);
    ...
    char **argv;
{
	...
	for (hPtr = Tcl_FirstHashEntry(&sourceTable, &cursor);
	    hPtr != NULL; hPtr = Tcl_NextHashEntry(&cursor)) {
	    tkwin = (Tk_Window)Tcl_GetHashKey(&sourceTable, hPtr);
    ...
    char **argv;
{
	...
	if (argc == 4) {
	     ...
	     */
	    hPtr = Tcl_CreateHashEntry(&(targetPtr->handlerTable), argv[4],
		&isNew);
	    cmd = Tcl_Concat(argc - 5, argv + 5);
	    if (hPtr != NULL) {
		char *oldCmd;

		oldCmd = (char *)Tcl_GetHashValue(hPtr);
		if (oldCmd != NULL) {
		    free(oldCmd);
		}
	    }
	...
	hPtr = Tcl_FindHashEntry(&(targetPtr->handlerTable), argv[4]);
	if (hPtr == NULL) {
    ...
    Tcl_Interp *interp;		/* interpreter to be updated */
{
    ...
    if (!initialized) {
	Tcl_InitHashTable(&sourceTable, TCL_ONE_WORD_KEYS);
