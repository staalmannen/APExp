...
Blt_FindChild(parent, name)
    ...
    char *name;
{
    register struct TkWindow *winPtr;
...
static void
UnlinkWindow(winPtr)
    struct TkWindow *winPtr;	/* Child window to be unlinked. */
{
    struct TkWindow *prevPtr;
 ...
 *		see blt::CreateTearoff in bltTabset.tcl.
 *
 * Results:
 *	None.
 *
...
Blt_RelinkWindow(tkwin, newParent, x, y)
    ...
    int x, y;
{
    struct TkWindow *winPtr, *parentWinPtr;

    if (Blt_ReparentWindow(Tk_Display(tkwin), Tk_WindowId(tkwin),
	    Tk_WindowId(newParent), x, y) != TCL_OK) {
	return;
    }
    winPtr = (struct TkWindow *)tkwin;
    parentWinPtr = (struct TkWindow *)newParent;
