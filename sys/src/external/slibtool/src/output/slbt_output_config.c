...
static int slbt_output_config_mkvars(const struct slbt_driver_ctx * dctx)
{
	...
	prefer_sltdl = (dctx->x_cctx->x_drvflags & SLBT_DRIVER_PREFER_SLTDL)
		? enable : disable;
	...
	if (slbt_dprintf(fdout,"# archive librarian\n" "RANLIB=\"%s\"\n\n",cctx->x_host.ha_ranlib) < 0)
		return SLBT_SYSTEM_ERROR(dctx);

	if (slbt_dprintf(fdout,"# assembler\n" "AS=\"%s\"\n\n",cctx->x_host.ha_as) < 0)
		return SLBT_SYSTEM_ERROR(dctx);

	if (slbt_dprintf(fdout,"# Shared library suffix\n" "shrext_cmds=\"%s\"\n\n",cctx->x_settings.fa_dsosuffix) < 0)
		return SLBT_SYSTEM_ERROR(dctx);

	if (slbt_dprintf(fdout,"# PE targets: modern import library compiler\n" "MDSO=%s%s%s\n\n",
			cctx->x_host.ha_mdso[0] ? "\"" : "",
			cctx->x_host.ha_mdso[0] ? cctx->x_host.ha_mdso : "",
			cctx->x_host.ha_mdso[0] ? "\"" : "") < 0)
		return SLBT_SYSTEM_ERROR(dctx);

	if (slbt_dprintf(fdout,"# PE targets: coff import library compiler\n" "DLLTOOL=%s%s%s\n\n",
			cctx->x_host.ha_dlltool[0] ? "\"" : "",
			cctx->x_host.ha_dlltool[0] ? cctx->x_host.ha_dlltool : "",
			cctx->x_host.ha_dlltool[0] ? "\"" : "") < 0)
		return SLBT_SYSTEM_ERROR(dctx);

	if (slbt_dprintf(fdout,"# PE targets: resource compiler\n" "WINDRES=%s%s%s\n\n",
			cctx->x_host.ha_windres[0] ? "\"" : "",
			cctx->x_host.ha_windres[0] ? cctx->x_host.ha_windres : "",
			cctx->x_host.ha_windres[0] ? "\"" : "") < 0)
		return SLBT_SYSTEM_ERROR(dctx);
