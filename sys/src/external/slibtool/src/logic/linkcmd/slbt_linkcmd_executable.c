	...
	const char *			exefilename)
{
	...
	/* fpic */
	fpic  = (dctx->x_cctx->x_drvflags & SLBT_DRIVER_SHARED);
	fpic &= !(dctx->x_cctx->x_drvflags & SLBT_DRIVER_PREFER_STATIC);
	...
	if (slbt_dprintf(fdwrap,
			...
			verinfo->v_commit,
			dctx->x_cctx->x_settings.fa_ldpathenv,
			dpfixup) < 0)
		return SLBT_SYSTEM_ERROR(dctx);
	...
	*ectx->x_lout[1] = output;
	...
	if (renameat(fdcwd,wrapper,fdcwd,dctx->x_cctx->x_output))
		return slbt_linkcmd_exit(
			&depsmeta,
			SLBT_SYSTEM_ERROR(dctx));

	if (fchmodat(fdcwd,dctx->x_cctx->x_output,0755,0))
		return slbt_linkcmd_exit(
			&depsmeta,
			SLBT_SYSTEM_ERROR(dctx));
