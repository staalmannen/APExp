...
void Handle_Const( char *arg_in) {
    ...
    char *str = NULL;
    if( AMOUNT(arg) == 1 ){
    fprintf(stderr, "%sSyntax error: empty CONST at line %s in file '%s'!\n", NL, STR( g_COUNTER), g_CURFILE);
        exit(1);
    }
    b2c_parse(&match, 1, &match__b2c_array, COLLAPSE(arg), "CONST *", b2c_option_delim);
...
fprintf(g_HFILE, "#define %s (%s)\n", var, str);
    if( REGEX(var, CONCAT( g_STRINGSIGN , "$") ) ){
        b2c_hash_add_str(assoc_g_MACRO_STRINGS, str, var);
    }
