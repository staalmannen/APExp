...
#undef b2c_exitval
#define b2c_exitval
void Handle_Readln( char *arg_in) {
    ...
    fprintf(stderr, "%sSyntax error: missing FROM in READLN statement at line %s in file '%s'!\n", NL, STR( g_COUNTER), g_CURFILE);
        exit(1);
    }
    b2c_parse(&match, 1, &match__b2c_array, COLLAPSE(arg), "READLN * FROM *", b2c_option_delim);
    var = b2c_CopyString(var, (char*) match[(uint64_t)1]);
    from = b2c_CopyString(from, (char*) match[(uint64_t)2]);
    if( NOT(Check_String_Type(var)) ){
    fprintf(stderr, "%sSyntax error: variable for READLN at line %s in file '%s' must be string!\n", NL, STR( g_COUNTER), g_CURFILE);
        exit(1);
    }
    if( REGEX(var, CONCAT( g_STRINGSIGN , "$") ) ){
        Register_Pointer(var, "char*");
    }
fprintf(g_CFILE, "b2c_readln(&b2c_assign, %s);\n", from);
