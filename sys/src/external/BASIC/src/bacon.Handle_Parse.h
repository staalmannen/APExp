...
void Handle_Parse( char *arg_in) {
    ...
    char *size = NULL;
    static char *by  = NULL;
    by  = b2c_CopyString(by ,  "b2c_option_delim");
    long use_static = 0;
    long total = 0;
    if( NOT(ISTOKEN(arg, "WITH")) ){
    fprintf(stderr, "%sSyntax error: missing WITH in PARSE statement at line %s in file '%s'!\n", NL, STR( g_COUNTER), g_CURFILE);
        exit(1);
    }
    if( NOT(ISTOKEN(arg, "TO")) ){
    fprintf(stderr, "%sSyntax error: missing TO in PARSE statement at line %s in file '%s'!\n", NL, STR( g_COUNTER), g_CURFILE);
        exit(1);
    }
    ...
    if( ISTOKEN(arg, "BY") ){
        b2c_parse(&match, 1, &match__b2c_array, COLLAPSE(arg), "PARSE * WITH * BY * TO * SIZE *", b2c_option_delim);
        by = b2c_CopyString(by, (char*) match[(uint64_t)3]);
        arg = F_APPEND(arg,HEAD(arg, ISTOKEN(arg, "BY")-1), 0, LAST(arg, ISTOKEN(arg, "TO")-1));
    }
    b2c_parse(&match, 1, &match__b2c_array, COLLAPSE(arg), "PARSE * WITH * TO * SIZE *", b2c_option_delim);
    total = match__b2c_array;
    source = b2c_CopyString(source, (char*) match[(uint64_t)1]);
    with = b2c_CopyString(with, (char*) match[(uint64_t)2]);
    ...
    fprintf(stderr, "%sSyntax error: variable %s in PARSE statement must be string at line %s in file '%s'!\n", NL, to, STR( g_COUNTER), g_CURFILE);
        exit(1);
    }
    if( LEN(size) ){
...
fprintf(g_CFILE, "b2c_parse(&%s, %s, &%s__b2c_array, %s, %s,%s);\n", to, STR( g_OPTION_BASE), to, source, with, by);
    if( LEN(size) ){
