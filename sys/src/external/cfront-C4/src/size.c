...
int get_line__FPv(void *__1fp) {
    ...
    if (fscanf(__1fp, (const char *)"%s %s", __1s2, __1s) == -1)
        return 0;

    if (strcmp((const char *)"DLARGEST_INT", (const char *)__1s) == 0) {

        if (fscanf(__1fp, (const char *)" %s", __1s2) == -1)
            return 0;
    ...
    } else if (fscanf(__1fp, (const char *)"%d ", &arg1) == -1)
        return 0;
    ...
    if (strcmp((const char *)"DLARGEST_INT", (const char *)__1s) == 0) {
        ...
        __2str_tmp =
            (((char *)__nw__FUl((size_t)((sizeof(char)) * (strlen((const char *)__1s2) + 1)))));
