...
/* find:  "int_file = alloca (sizeof (struct file))" */
                  ...
                  if (intermed_ok)
                    {
                      ...
                      if (int_file == 0)
 /* line changed the same way as lines were changed in function.c to avoid typedef conflicts */
                        int_file = (struct file *) alloca (sizeof (struct file));
/* below everything should be normal */
