...
tcomo(Node *n, int f)
{
	...
	case OADDR:
		if(tcomo(l, ADDROP))
			goto bad;
		/*
		 * &(a, b) => (a, &b).  compoundlit() lowers a C99 compound
		 * literal to OCOMMA(init, ONAME); for struct/union (and
		 * complex) types that OCOMMA cannot be marked addable,
		 * because those are generated by sugen(), not cgen(), so
		 * &(struct S){...} would fail the l-value test below.
		 * Hoisting the comma out leaves OADDR applied to the hidden
		 * temporary's ONAME, which is addressable in every case.
		 */
		if(l->op == OCOMMA && l->right != Z && l->right->addable
		&& l->right->type != T && l->right->type->nbits == 0) {
			r = new1(OADDR, l->right, Z);
			r->lineno = n->lineno;
			r->type = typ(TIND, l->right->type);
			r->type->width = types[TIND]->width;
			n->op = OCOMMA;
			n->left = l->left;
			n->right = r;
			n->type = r->type;
