...
rcmd(char *arname, int count, char **files)
{
	...
	for(i = 0; fd >= 0; i++) {
		...
		if(dupfound) {
			/* duplicate symbol: skip replacement, keep old member absent */
			dupfound = 0;
			free(d);
			Bterm(bfile);
			continue;
	...
	for (i = 0; i < count; i++) {
		...
		if (!bfile)
			...
			if (d == nil)
				...
				if(dupfound) {
					/* duplicate symbol: skip this member, continue with rest */
					dupfound = 0;
					free(d);
					Bterm(bfile);
					continue;
...
install(char *arname, Arfile *astart, Arfile *amiddle, Arfile *aend, int createflag)
{
	...
	/* dupfound is only a warning: the duplicate symbol was already excluded
	 * from the symdef by objsym(), so the archive is still consistent.
	 * Do not abort - continue writing so all non-duplicate members are kept. */
