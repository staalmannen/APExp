...
/*
 * VLA frame management for Power64 (32-bit ABI, qc).
 *
 * Same structure as 9c (PowerPC 32-bit): use R30 as a callee-saved
 * VLA frame anchor register (not used by the qc register allocator).
 *
 * gvla_prologue – at function entry when vlanest > 0:
 *   Save R30, then set R30 = SP.
 *
