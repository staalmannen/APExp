...
/*
 * VLA frame management for Motorola 68k (1c/2c).
 *
 * The 68k linker emits "LINK A6,#-framesize / UNLK A6" for every
 * function, so A6 is already the frame pointer.  SP = A7.
 *
 * We use A5 (D_A0+5) as the VLA stack anchor.  A5 sits just above
 * the allocatable address register range and is callee-saved in the
 * 68k ABI.  We claim it by incrementing aregused[5] so the register
 * allocator will not touch it.
 *
 * gvla_prologue – at function entry when vlanest > 0:
 *   Save A5, then set A5 = A7 to anchor the pre-VLA stack top.
 *
 * gvla_epilogue – before every RET when vlanest > 0:
 *   Restore A7 from A5, restore A5.
 *
...
gvla_prologue(void)
{
	...
	sp = D_A0 + 7;		/* A7 = SP */
	fp = D_A0 + 5;		/* A5 = VLA frame anchor */
	aregused[5]++;		/* claim A5 */

	/* MOVL A5, -(A7)  — push A5 onto stack */
	nextpc();
	p->as = AMOVL;
	p->from.type = fp;
...
gvla_epilogue(void)
{
	...
	/* MOVL A5, A7  — restore SP to pre-VLA position */
	nextpc();
	p->as = AMOVL;
	p->from.type = fp;
	p->to.type = sp;
