...
/*
 * VLA frame management for SPARC.
 *
 * SPARC uses a register-window ABI where SAVE establishes the frame.
 * SP (%o6, R1 in Plan9 kc numbering) can be further decremented for
 * dynamic allocation.  We use R30 (a local register, callee-saved
 * across the SAVE/RESTORE window pair) as the VLA frame anchor.
 *
 * gvla_prologue – at function entry when vlanest > 0:
 *   Save R30, then set R30 = SP to anchor the pre-VLA stack top.
 *
 ...
 */
enum { REGVFP = 30 };	/* VLA frame pointer: callee-saved local reg */
