#ifndef _APEXP_LIMITS_GENERIC_H_
#define _APEXP_LIMITS_GENERIC_H_

/*
 * The content of <limits.h>, under a name none of the copies of
 * limits.h can reach by their own name -- stdint_generic.h's
 * arrangement, and for the same reason. See the wrapper for why.
 */

/* the marker: a test can ask which limits.h a compile actually read */
#define _APEXP_LIMITS_H 1
 ...
 * MINIMA -- AND UNTIL THE WRAPPER BESIDE THIS FILE EXISTED, NEITHER
 * NUMBER HERE WAS EVER READ. Stock APE's copy in the architecture
 * directory won the search and said PATH_MAX 1023, NAME_MAX 27; this
 * file said 255 and 14. `deeppath-test` printed 1023/27 on a build
 * with 4096/255 sitting in this file, which is how that was found.
 * The 1023 is a real wall and the 27 is simply wrong -- <dirent.h>
 * gives `struct dirent` a `d_name[MAXNAMLEN+1]` with MAXNAMLEN 255.
 * `_POSIX_NAME_MAX` (14) and `_POSIX_PATH_MAX` (255) are what
